"""将/json_msg的内容转换为chatglm3所需要的数据集格式(已过时)"""
import json
import constants
import loguru
import os
...
def convert_to_dataset(chat_groups, my_username, their_username):
    ...
    for chat_group in chat_groups:
        ...
        if history is not None and len(history) > 0:
            chatglm3_dataset.append({
                "conversations": conversation
            })
    return chatglm3_dataset
...
def main():
    ...
    output_dir = os.path.join(os.path.dirname(__file__), "chatglm3_datasets")
    ...
    # file_path = "./json_msg/652076201.txt.json"
...
if __name__ == "__main__":
    main()
    dataset = merge_datasets(os.path.join(os.path.dirname(__file__), "chatglm3_datasets"))
    with open(os.path.join(os.path.dirname(__file__), "chatglm3_qq_dataset.json"), "w", encoding="utf-8") as f:
        json.dump(dataset, f, ensure_ascii=False, indent=4)
